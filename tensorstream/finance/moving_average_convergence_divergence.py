...
class MovingAverageConvergenceDivergence(Streamable):
  def __init__(self, slow, fast, macd, dtype=tf.float32, shape=()):
    ...
    self._ema_slow = ExponentialMovingAverage(
      self.slow_period, dtype=dtype, shape=shape)
    self._ema_fast = ExponentialMovingAverage(
      self.fast_period, dtype=dtype, shape=shape)
    self._ema_macd = ExponentialMovingAverage(
      self.macd_period, dtype=dtype, shape=shape)

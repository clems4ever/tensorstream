...
from tensorstream.operators.moving_average import ExponentialMovingAverage
from tensorstream.operators.moving_average import RollingMovingAverage
from tensorstream.operators.tests import TestCase

class SimpleMovingAveragesSpec(TestCase):
  def setUp(self):
    self.sheets = self.read_ods(
      self.from_test_res('moving_averages_sma.ods'))
...
class ExponentialMovingAverageSpec(TestCase):
  ...
  def setUp(self):
    self.sheets = self.read_ods(
      self.from_test_res('moving_averages_ema.ods'))
...
class RollingMovingAverageSpec(TestCase):
  def setUp(self):
    self.sheets = self.read_ods(
      self.from_test_res('moving_averages_rma.ods'))
